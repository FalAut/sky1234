...
onEvent("ponder.registry", (ponder) => {
  ponder
    ...
    .scene("nether_portal_frame", "Nether Portal Frame", (scene, util) => {
      ...
      scene
        .showControls(40, [2, 2.5, 3], "down")
        .withItem("kubejs:nether_eye")
        .rightClick();
      scene.idle(10);
      for (let x = 2; x >= 1; x--) {
      ...
      scene
        .showControls(20, [3.5, 0.5, 3.5], "left")
        .withItem("naturesaura:token_anger")
        .rightClick();
      scene.idle(20);
      scene.world.setBlocks(
        [1, 1, 1, 2, 1, 2],
        "customportalapi:custom_portal_block",
        false
      );
      scene.world.modifyBlocks(
        [1, 1, 1, 2, 1, 2],
        (curState) => curState.with("axis", "y"),
        false
      );
    });
});
