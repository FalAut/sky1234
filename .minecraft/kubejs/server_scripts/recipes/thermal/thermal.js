...
onEvent("recipes", (event) => {
  ...
  pyrolyzer(
    [Fluid.of("immersiveengineering:creosote", 900), "thermal:coal_coke_block"],
    "coal_block"
  ).id(`${id_prefix}coal_coke_block`);

  pyrolyzer(
    [Fluid.of("immersiveengineering:creosote", 125), "minecraft:charcoal"],
    "#minecraft:logs"
  ).id(`${id_prefix}logstocreosote`);

  smelter("9x occultism:iesnium_ingot", [
    "#forge:raw_materials",
    "create:refined_radiance",
  ]).id(`${id_prefix}iesnium_ingot`);

  smelter("immersiveengineering:ingot_steel", [
    "iron_ingot",
    "immersiveengineering:coal_coke",
  ...
  smelter("16x pipez:item_pipe", [
    "minecraft:dropper",
    "4x thermal:cured_rubber",
  ]).id(`${id_prefix}item_pipe`);

  smelter("16x pipez:fluid_pipe", ["bucket", "4x thermal:cured_rubber"]).id(
    `${id_prefix}fluid_pipe`
  );

  smelter("16x pipez:energy_pipe", [
    "immersiveengineering:wirecoil_copper",
    "4x thermal:cured_rubber",
  ]).id(`${id_prefix}energy_pipe`);

  press("thermal:diamond_gear", ["4x diamond", "thermal:press_gear_die"]).id(
    `${id_prefix}diamond_gear`
  );

  crucible(Fluid.of("tconstruct:molten_diamond", 100), "diamond").id(
    `${id_prefix}molten_diamond`
  );
