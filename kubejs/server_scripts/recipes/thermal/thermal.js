onEvent("recipes", (event) => {
  ...
  const { pyrolyzer, crystallizer, furnace, sawmill, pulverizer, smelter, centrifuge, chiller, refinery, brewer, bottler, press, crucible, insolator, pulverizer_catalyst, smelter_catalyst, insolator_catalyst } = event.recipes.thermal
  ...
  smelter("thermal_extra:soul_infused_ingot", ["thermal_extra:sticky_ball", "spirit:soul_steel_ingot"]).id(`${id_prefix}soul_infused_ingot`)

  smelter("thermal_extra:shellite_ingot", ["architects_palette:oracle_jelly", "thermal:enderium_ingot"]).id(`${id_prefix}shellite_ingot`)

  smelter("create:chromatic_compound", ["thermal_extra:twinite_ingot", "thermal_extra:soul_infused_ingot", "thermal_extra:shellite_ingot"]).id(`${id_prefix}chromatic_compound`)
  ...
  smelter("industrialforegoing:machine_frame_simple", ["industrialforegoing:machine_frame_pity", "4x industrialforegoing:plastic", "2x create:shadow_steel"]).id(`${id_prefix}machine_frame_simple`)

  smelter("immersiveengineering:circuit_board", ["immersiveengineering:plate_duroplast", "immersiveengineering:insulating_glass"]).id(`${id_prefix}circuit_board`)

  press(Item.of("kubejs:incomplete_rf_coil", '{SequencedAssembly:{Progress:0.2f,Step:1,id:"kubejs:recipes/create/sequenced_assembly/rf_coil_thermal"}}'), ["gold_ingot", "kubejs:mold_rf_coil"]).id(`${id_prefix}incomplete_rf_coil`)

  press(Item.of("kubejs:incomplete_redstone_servo", '{SequencedAssembly:{Progress:0.2f,Step:1,id:"kubejs:recipes/create/sequenced_assembly/redstone_servo_thermal"}}'), ["iron_ingot", "kubejs:mold_redstone_servo"]).id(`${id_prefix}incomplete_redstone_servo`)
  ...
  press("assemblylinemachines:attuned_titanium_plate", "assemblylinemachines:attuned_titanium_ingot").id(`${id_prefix}attuned_titanium_plate`)
  ...
  press("assemblylinemachines:pure_copper_gear", ["4x assemblylinemachines:pure_copper_ingot", "thermal:press_gear_die"]).id(`${id_prefix}pure_copper_gear`)

  press("assemblylinemachines:pure_gold_gear", ["4x assemblylinemachines:pure_gold_ingot", "thermal:press_gear_die"]).id(`${id_prefix}pure_gold_gear`)

  press("assemblylinemachines:pure_iron_gear", ["4x assemblylinemachines:pure_iron_ingot", "thermal:press_gear_die"]).id(`${id_prefix}pure_iron_gear`)

  press("assemblylinemachines:pure_steel_gear", ["4x assemblylinemachines:pure_steel_ingot", "thermal:press_gear_die"]).id(`${id_prefix}pure_steel_gear`)

  press("assemblylinemachines:pure_titanium_gear", ["4x assemblylinemachines:pure_titanium_ingot", "thermal:press_gear_die"]).id(`${id_prefix}pure_titanium_gear`)

  press("assemblylinemachines:novasteel_gear", ["4x assemblylinemachines:novasteel_ingot", "thermal:press_gear_die"]).id(`${id_prefix}novasteel_gear`)

  press("assemblylinemachines:mystium_gear", ["4x assemblylinemachines:mystium_ingot", "thermal:press_gear_die"]).id(`${id_prefix}mystium_gear`)
